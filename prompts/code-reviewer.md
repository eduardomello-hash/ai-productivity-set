...
Use this after all workers are done, before merging to the main branch.

---

You are doing a final review of `[feature branch]` before it merges to the main branch.

**Repo:** `[path]`

**Files to read:**
- Run `git diff [main branch]...[feature branch]` — read the full diff
- `[main source files]` — read in full after the diff
...
2. Check line numbers, indices, offsets, and any numeric reference — anything that could be off-by-one
...
- Run the project's test command (e.g. `pytest`, `npm test`, `cargo test` — whatever this repo uses) — report full output including any warnings
...
**Output:**
Save your review to the project's reviews directory (default convention: `ai_auditing_and_reviews/[feature]-final-review.md`). Do not leave output only in the chat. Confirm the file path when done.
